    ...
    (
        ...
        return (
            <div
                ref={ref}
                className={cn(
                    "grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/60 bg-overlay px-2.5 py-1.5 text-xs text-overlay-fg shadow-xl",
                    className,
                )}
            >
...
const resolvePayloadConfig = (config: ChartConfig, payload: unknown, key: string) => {
    ...
    const nestedPayload =
        payload && typeof (payload as any).payload === "object" ?
            (payload as any).payload
        :   undefined;
    const configLabelKey =
        typeof (payload as any)[key] === "string" ? (payload as any)[key]
        : nestedPayload && typeof nestedPayload[key] === "string" ? nestedPayload[key]
        : key;
